...
def init_db(conn: sqlite3.Connection) -> None:
    conn.executescript(
        ...
        CREATE TABLE IF NOT EXISTS predictions (
            ...
            schedule_total_flights INTEGER,
            schedule_completed_flights INTEGER,
            schedule_unavailable_flights INTEGER,
            schedule_pending_flights INTEGER,
            schedule_active_flight_index INTEGER,
            schedule_active_flight_hour INTEGER,
            schedule_active_flight_time TEXT,
            schedule_active_flight_numbers TEXT,
            schedule_active_flight_status TEXT,
...
async def make_prediction_rows(conn: sqlite3.Connection, horizons: list[int], timezone_name: str) -> int:
    ...
    for horizon in horizons:
        ...
        has_final_board_status = schedule.available and (schedule.moved_next_day or schedule.completed_same_day)
        if horizon <= OPEN_METEO_MAX_HORIZON_DAYS and not weather.available and not has_final_board_status:
            logger.warning(
        ...
        cursor = conn.execute(
            ...
            INSERT INTO predictions (
                ...
                schedule_status_summary, schedule_total_flights, schedule_completed_flights,
                schedule_unavailable_flights, schedule_pending_flights, schedule_active_flight_index,
                schedule_active_flight_hour, schedule_active_flight_time, schedule_active_flight_numbers,
                schedule_active_flight_status,
            ...
            ON CONFLICT(run_date, target_date, model_version, data_version) DO UPDATE SET
                ...
                schedule_total_flights = excluded.schedule_total_flights,
                schedule_completed_flights = excluded.schedule_completed_flights,
                schedule_unavailable_flights = excluded.schedule_unavailable_flights,
                schedule_pending_flights = excluded.schedule_pending_flights,
                schedule_active_flight_index = excluded.schedule_active_flight_index,
                schedule_active_flight_hour = excluded.schedule_active_flight_hour,
                schedule_active_flight_time = excluded.schedule_active_flight_time,
                schedule_active_flight_numbers = excluded.schedule_active_flight_numbers,
                schedule_active_flight_status = excluded.schedule_active_flight_status,
