...
- Open-Meteo hourly-прогноз агрегируется не за все сутки, а за рабочее окно `WEATHER_FORECAST_WINDOW_START_HOUR`-`WEATHER_FORECAST_WINDOW_END_HOUR` по локальному времени аэропорта. По умолчанию это `08:00-20:00`, чтобы не переоценивать ночной или ранний утренний туман, если к дневному/вечернему окну условия улучшаются.
- Для `visibility` используется нижний квартиль внутри рабочего окна, а не абсолютный суточный минимум. Длительная плохая видимость всё ещё влияет на fog-risk, но одиночный плохой час не должен автоматически ломать прогноз на весь день.
...
- MET Norway fallback агрегируется в том же рабочем окне, но не содержит прямой `visibility`, поэтому fog-risk в этом режиме считается по доступным proxy-признакам: низкая облачность, влажность, dew point spread, ветер, осадки и `fog_area_fraction`, если он есть.
...
- Baseline `mvp-baseline-003` использует эти признаки как дополнительную отрицательную поправку на риск тумана и низкой облачности.
